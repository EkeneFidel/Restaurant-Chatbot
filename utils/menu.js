...
const { sendMenu, sendFoodMenu, pushMessage } = require("./message");
...
const handleUserPrompt = async (
    ...
    switch (userPrompt) {
        ...
        default:
            let validPrompts = menuItems["menuPrompts"].map(function (n) {
...
const handleUserOrder = (userPrompt, socket, sessionId, currentOrder) => {
    ...
    let mainMenu;
    let totalPrice;
    let validPrompts = menuItems["foodItems"].map(function (n) {
        return n.id.toString();
    });
    ...
    if (!validPrompts.includes(userPrompt)) {
        ...
        const selectedItem = menuItems["foodItems"].find(
        ...
        let botMessage = formatMessage(
            "bot",
            `1 ${selectedItem.item} added to your current order`,
            sessionId
        );
        pushMessage(socket, sessionId, botMessage);
        sendMenu(socket, sessionId);
